...
- **Unmatched records are dropped silently**: if no pipeline in the merge group accepts a record, it's skipped (logged at `debug` level only, so invisible under the default `info` log level). A preset picks which record types to transfer — types outside the preset's filter set are intentionally left behind, but be aware that the transfer isn't exhaustive by default. If you need every record to land on the target, add a catch-all pipeline last (`.filter(createFilter(() => true))`) or register a zero-transformer passthrough under the same scanner.
