...
│   ├── PipelineBuilderFactory/      # Injects all Processor + Scanner instances (multiple: true deps); .create({name, scanner, processors})
...
- **Unmatched records are dropped per design**: if NO pipeline in the merge group accepts a record, it's skipped. Each unmatched record emits a `warn`-level log line: `"unmatched record — TYPE=<type> PK=<pk> SK=<sk>"`. The shard summary shows a TYPE breakdown: `"unmatched 5 (page.page=3, cms.entry=2)"`. After the run, `segment-N-unmatched.log` under `.transfer/<runId>/` lists every unmatched record (one per line: `[TYPE] PK : SK`). If users want exhaustive transfer, register a catch-all passthrough pipeline last.
