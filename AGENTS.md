...
- **Unmatched records are silently dropped**: if NO pipeline in the merge group accepts a record, it's skipped and logged at `debug` level only (suppressed under the default `info` log level). A transfer CAN silently leave a chunk of source records behind — this is by design (the preset picks which types to transfer) but surprising on a real prod run. No aggregate count today; `runShard` logs per-drop only. If users care about visibility, filter everything through a catch-all pipeline at the end of registration.
