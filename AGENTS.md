...
- `PipelineBuilderFactory.create({ name, scanner, processors })` accepts Implementation classes (not just abstractions). The type system infers `TRecord` / `TContext` / `TShard` from the Impl class instance types. At runtime the factory calls `container.resolveAll(Processor)` and matches each impl class by `p.constructor === implClass` to get the correct singleton — **pipelines carry resolved processor instances, not tokens**. All processors share `Symbol("Core/Processor")` as their abstraction token; the factory resolves the right one by constructor identity, not by last-registered.
