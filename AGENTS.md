...
│   ├── PipelineBuilderFactory/      # Injects all Processor instances (multiple: true dep); .create({name, scanner, processors})
│   │                                # finds the right instance by constructor identity → PipelineBuilder
...
- `PipelineBuilderFactory.create({ name, scanner, processors })` accepts Implementation classes (not just abstractions). The type system infers `TRecord` / `TContext` / `TShard` from the Impl class instance types. At runtime the factory has all `Processor` instances injected via `[Processor, { multiple: true }]` dependency, then finds the right one by `p.constructor === implClass` — **pipelines carry resolved processor instances, not tokens**. All processors share `Symbol("Core/Processor")`; constructor identity is the discriminator, not registration order.
