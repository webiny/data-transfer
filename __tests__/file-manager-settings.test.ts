...
interface KeyValueStoreRecord extends BaseRecord {
    GSI_TENANT?: string;
    data: {
        key?: string;
        scope?: string;
...
describe("File Manager Settings", () => {
    it("should migrate FM settings to KeyValue format", async () => {
        const container = createDdbContainer({
            sourceRecords: { "source-table": [v5FileManagerSettings as BaseRecord] }
        });
        ...
        const migratedRecord = migratedRecords[0] as KeyValueStoreRecord;
        ...
        const value = migratedRecord.data.value;
        expect(value).toBeDefined();
        expect(value?.srcPrefix).toBe("https://d8eqa02y4s7ns.cloudfront.net/files/");
        expect(value?.uploadMaxFileSize).toBe(10737418240);

        expect(value?.tenant).toBeUndefined();
