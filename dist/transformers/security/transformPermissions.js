 ...
 */
export const transformPermissions = createTransformer("transformPermissions", async ctx => {
  const { record } = ctx;
  ...
  const defaultLocale = extractDefaultLocale(record.PK);
    ...
    if (permission.name === "cms.contentModelGroup" && permission.groups) {
      if (typeof permission.groups === "object" && !Array.isArray(permission.groups)) {
        ...
        for (const groupId of groupIds) {
          const groupRecord = await ctx.querySourceRecord(`T#${tenant}#GROUP#${groupId}`, "A");
...
function extractDefaultLocale(pk) {
  // Extract locale from PK like "T#root#L#en-US#GROUP#..."
  const match = pk.match(/#L#([^#]+)#/);
  return match ? match[1] : "en-US";
}
//# sourceMappingURL=transformPermissions.js.map
