...
export const extractImageMetadata = createDdbTransformer("extractImageMetadata", async ctx => {
  ...
  const s3Key = (await resolveFileKeyFromDb(ctx, data)) || values["text@key"];
  if (!s3Key) {
    values["object@metadata"] = {};
    return;
  }
  try {
    ...
    ctx.logger.error(`[extractImageMetadata] key="${s3Key}": ${err.message}`);
    values["object@metadata"] = {};
  }
});
function getFileId(data) {
  const id = data.id || data.entryId;
  return id?.replace(/#\d+$/, "");
}
/** Check source DB for an existing KV metadata record (subsequent migration run). */
async function resolveFileKeyFromDb(ctx, data) {
  const fileId = getFileId(data);
  if (!fileId) {
    return undefined;
  }
...
//# sourceMappingURL=extractImageMetadata.js.map
