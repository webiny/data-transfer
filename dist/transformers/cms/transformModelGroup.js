 ...
 */
export const transformModelGroup = createTransformer("transformModelGroup", async ctx => {
  ...
  const groupRecord = await ctx.querySourceRecord(`T#${tenant}#L#${locale}#CMS#CMG`, group.id);
  if (groupRecord && groupRecord.slug) {
    ...
    ctx.logger.warn(`[transformModelGroup] Group ${group.id} not found, using name as fallback`);
    data.group = group.name ? group.name.toLowerCase().replace(/\s+/g, "-") : "ungrouped";
  }
});
//# sourceMappingURL=transformModelGroup.js.map
