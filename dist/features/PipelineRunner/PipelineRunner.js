...
import { PipelineRunner as PipelineRunnerAbstraction } from "./abstractions/PipelineRunner.js";
class PipelineRunnerImpl {
  ...
  constructor(
    ...
    this.transferredLog = transferredLog;
  }
  register(...pipelines) {
  ...
  async runSingleShard(scanner, pipelines, opts) {
  ...
  async runMergeGroup(scanner, pipelines) {
    const mergeGroupId = this.deriveMergeGroupId(scanner);
    const hookParams = {
  ...
  resolvePipelineProcessors(pipelines) {
  ...
  dedupHookTokens(pipelines, lifecycle) {
    const seen = new Set();
    const result = [];
    for (const pipeline of pipelines) {
      const tokens = lifecycle === "before" ? pipeline.beforeHookTokens : pipeline.afterHookTokens;
  ...
  async runShard(params) {
    ...
    const perPipelineTransferred = new Map();
    const perPipelineBlackholed = new Map();
    ...
    for await (const record of scanner.scan(shard)) {
      ...
      if (!matched) {
        ...
        await this.snapshotWriter.write(`dropped/segment-${shardCtx.segment}.jsonl`, record);
  ...
  async flushShard(commands, processors) {
    for (const processor of processors) {
      await processor.execute(commands);
    }
    this.warnUnclaimedKeys(commands);
  }
  async runRecord(pipeline, processors, record, shardCommands, shardCtx) {
    ...
    const { ctx, commands } = this.baseContextFactory.create({ record });
  ...
  collectProcessorOrder(pipelines, pipelineProcessors) {
    const seen = new Set();
    const ordered = [];
    for (const pipeline of pipelines) {
      const processors = pipelineProcessors.get(pipeline);
  ...
  logShardSummary(mergeGroupId, shardCtx, transferred, blackholed, unmatched) {
  ...
  logRunSummary(mergeGroupId, stats) {
    const transferred = new Map();
    const blackholed = new Map();
